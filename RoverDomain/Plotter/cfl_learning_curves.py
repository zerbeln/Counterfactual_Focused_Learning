...
def generate_cfl_learning_curves(generations, sample_rate, sruns, n_poi, coupling):
    ...
    plt.legend(["Global", "Difference", "D++", "CFL-High", 'CFL-Team', 'CFL-Low'], ncol=3, bbox_to_anchor=(0.5, 1.13), loc='upper center', borderaxespad=0)

    # Save the plot
    if not os.path.exists('Plots'):  # If Data directory does not exist, create it
        os.makedirs('Plots')
    plt.savefig(f"Plots/CFL_{n_poi}POI_C{coupling}.pdf")
...
if __name__ == '__main__':
    ...
    generate_cfl_learning_curves(generations, sample_rate, sruns, n_poi, coupling)
