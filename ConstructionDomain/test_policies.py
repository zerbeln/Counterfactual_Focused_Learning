...
from global_functions import load_saved_policies, create_csv_file, create_pickle_file
import numpy as np
...
def test_trained_policy(config_id):
    ...
    """
    # World Setup
    rd = ConstructionDomain()  # Create instance of the rover domain
    rd.load_world()
    ...
    while srun < p["stat_runs"]:
        ...
        for step_id in range(p["steps"]):
            ...
            for ds_id in range(p["n_dog_sites"]):
                ds_rewards[ds_id, step_id] = step_rewards[ds_id]
                if rd.dig_sites[f'P{ds_id}'].hazardous:
                    for dist in rd.dig_sites[f'P{ds_id}'].observer_distances:
