...
def generate_cfl_coupling_plots(generations, sample_rate, sruns):
    ...
    # Plot of Data
    plt.plot(x_axis, g_reward, color=color2)
    plt.plot(x_axis, d_reward, color=color1)
    ...
    # Plot of Error
    alpha_val = 0.2
    # plt.fill_between(x_axis, g_reward + g_stdev, g_reward - g_stdev, alpha=alpha_val, facecolor=color2)
    # plt.fill_between(x_axis, d_reward + d_stdev, d_reward - d_stdev, alpha=alpha_val, facecolor=color1)
    ...
    plt.legend(["Global", "Difference", "D++", "CFL-High", 'CFL-Team', 'CFL-Low'], ncol=4, bbox_to_anchor=(0.5, 1.13), loc='upper center', borderaxespad=0)

    # Save the plot
    if not os.path.exists('Plots'):  # If Data directory does not exist, create it
        os.makedirs('Plots')
    plt.savefig("Plots/CFL_Coupling_Perf.pdf")
...
if __name__ == '__main__':
    ...
    generate_cfl_coupling_plots(generations, sample_rate, sruns)
