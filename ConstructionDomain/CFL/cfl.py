...
def rover_cdif():
    ...
    counterfactuals = []
    if p["c_type"] == "Auto":
        counterfactuals = generate_high_low_counterfactuals(rd.dig_sites)
    elif p["n_dig_sites"] == 10:
        counterfactuals = ten_ds_counterfactuals()
    elif p["n_dig_sites"] == 5:
        counterfactuals = five_ds_counterfactuals()
    elif p["n_dig_sites"] == 4:
        counterfactuals = four_ds_counterfactuals()
    elif p["n_dig_sites"] == 2:
        counterfactuals = two_ds_counterfactuals()
        ...
        for gen in range(p["generations"]):
            ...
            for team_number in range(p["pop_size"]):
                ...
                for cf_id in range(p["n_configurations"]):
                    ...
                    for p_reward in ds_rewards:
                        g_reward += max(p_reward)
                    cfl_d_rewards, g_count = calc_cfl_difference(rd.dig_sites, g_reward, rd.agent_ds_distances, counterfactuals)
        ...
        for gen in range(p["generations"]):
            ...
            for team_number in range(p["pop_size"]):
                ...
                for cf_id in range(p["n_configurations"]):
                    ...
                    for p_reward in ds_rewards:
                        g_reward += max(p_reward)
                    cfl_dpp_rewards, g_count = calc_cfl_dpp(rd.dig_sites, g_reward, rd.agent_ds_distances, counterfactuals)
                    gen_g_count += g_count  # Increment calls to g counter
                    for rover_id in range(p["n_rovers"]):
                        policy_id = int(pops[f'EA{rover_id}'].team_selection[team_number])
